...
terraform {
  ...
  backend "s3" {
    ...
    key    = "common/github-actions/terraform.tfstate"
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:s3:::${var.project_name}-v2-artifact-*/*",
          "arn:aws:s3:::${var.project_name}-v3-artifact-*",
          "arn:aws:s3:::${var.project_name}-v3-artifact-*/*"
        ]
      },
      {
...
resource "aws_iam_policy" "ssm_run_command" {
  name        = "${var.project_name}-SSM-Run-Command"
  description = "Allow running SSM commands on managed EC2 instances"
...
resource "aws_iam_user_policy_attachment" "github_actions_ssm" {
  user       = aws_iam_user.github_actions.name
