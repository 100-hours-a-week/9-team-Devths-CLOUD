...
variable "nat_type" {
  ...
  type        = string
  default     = "gateway" # prod는 NAT Gateway 사용
}

variable "single_nat" {
  description = "Use a single NAT in ap-northeast-2a only (cost optimization)"
  type        = bool
  default     = true # prod는 단일 NAT Gateway (ap-northeast-2a) 사용
}

variable "nat_instance_type" {
  description = "Instance type for NAT instance (not used for gateway)"
  type        = string
  default     = "t3.nano"
}
