...
module "rds" {
  ...
  allowed_security_group_ids = [data.terraform_remote_state.network.outputs.be_security_group_id]
  ...
  skip_final_snapshot     = true                  # Dev 환경은 final snapshot 생성 안함

  # 고가용성 설정 (Dev 환경은 비활성화)
  multi_az            = false
  availability_zone   = "ap-northeast-2a" # 비용 절감을 위해 단일 AZ 사용
  publicly_accessible = false

  # 성능 및 모니터링 (Dev 환경은 비활성화로 비용 절감)
